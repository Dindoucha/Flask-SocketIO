...
from flask import jsonify, make_response, render_template_string, request, Blueprint
from flask_cors import cross_origin
from flask_jwt_extended import (create_access_token, create_refresh_token,get_jwt_identity,
                                jwt_required, unset_jwt_cookies, set_access_cookies,
                                set_refresh_cookies)
from flask_mail import Mail, Message
from app.models import Account, ResetToken, Game
...
@routes_bp.route("/get_player", methods=["POST", "GET"])
@jwt_required()
def get_player():
    player = get_jwt_identity()
    return make_response(jsonify(player), 200)

@routes_bp.route("/creategame", methods=["POST", "GET"]) # create a new game for logged-in user
@jwt_required()
def newgame_auth():
...
@routes_bp.route("/game", methods=["POST", "GET"]) # validate and save moves for logged-in user
@jwt_required()
def game():
    current_user = get_jwt_identity()
...
@routes_bp.route("/game_noauth", methods=["POST", "GET"]) # validate and save game for not-auth user
def game_noauth():
    data = request.get_json()
    fen = data.get("fen")   
    board = chess.Board()
...
@routes_bp.route("/profile/games", methods=["GET"]) # show player's previous and ongoing games
@jwt_required()
def mygames():
    current_user = get_jwt_identity()
...
@routes_bp.route("/loggedhome", methods=["GET", "POST"])
def home():
    response = {"status":"ok"}
    return make_response(jsonify(response), 200)
...
@routes_bp.route("/profile/update", methods=["GET", "POST"])
@jwt_required()
...
@routes_bp.route("/profile/delete", methods=["DELETE"])
@jwt_required()
@cross_origin(supports_credentials=True)
def delete_account():
    current_user = get_jwt_identity()
...
@routes_bp.route("/reset_send_email", methods=["GET", "POST"])
def reset():
    try:
        data = request.get_json()
        email = data.get("email")
...
@routes_bp.route("/check_auth", methods=["GET"])
@jwt_required()
def checking():
    response = {"auth": True}
    return make_response(jsonify(response), 200)